            ...
            return new Promise(async (resolve, reject) => {
                ...
                for (let i = 0; i < processingFiles.length; i++) {
                    ...
                    try {
                        ...
                        try {
                            let compressedFile = await imageCompression(currentFile.file, options);

                            // Exact Target KB Strict Validation Loop
                            if (currentFile.settings.targetMode && options.maxSizeMB) {
                                let targetBytes = options.maxSizeMB * 1024 * 1024;
                                let limitBytes = targetBytes; // rigorous exact match
                                if (compressedFile.size > limitBytes) {
                                    let loops = 0;
                                    let currentWidth = 1920; 
                                    while (compressedFile.size > limitBytes && loops < 7) {
                                        options.maxWidthOrHeight = currentWidth;
                                        options.useWebWorker = true;
                                        try {
                                            compressedFile = await imageCompression(currentFile.file, options);
                                            currentWidth = Math.floor(currentWidth * 0.85); // actively force dim reduction
                                        } catch (e) {
                                            break;
                                        }
                                        loops++;
                            ...
                            if (compressedFile.size > currentFile.file.size && currentFile.settings.outputFormat === "original") {
                                compressedFile = currentFile.file;
                            }
                            ...
                            if (isMobile) {
                                try {
                                    let fallbackBlob = await mobileCanvasFallback(currentFile.file, {
                                    ...
                                    // Prevent fallback inflation
                                    if (fallbackBlob.size > currentFile.file.size && currentFile.settings.outputFormat === "original") {
                                        fallbackBlob = currentFile.file;
                                    }
